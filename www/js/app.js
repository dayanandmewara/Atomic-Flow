                ...
                <div class="glass-card reward-item-card" style="display: flex; flex-direction: column; justify-content: space-between; padding: 1.25rem; border-radius: var(--radius-md); transition: transform 0.2s, box-shadow 0.2s; position: relative;">
                    <div style="display: flex; align-items: flex-start; gap: 12px;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 4px; min-width: 0;">
                            ...
                            ${r.packageName ? `
                                <span style="font-size: 0.68rem; color: var(--text-muted); display: flex; align-items: center; gap: 4px; font-weight: 600; margin-top: 1px;">
                                    <i data-lucide="smartphone" style="width: 10px; height: 10px; color: var(--primary);"></i> Unlocks ${r.packageName.length > 20 ? r.packageName.substring(0, 18) + '...' : r.packageName} (${r.durationMinutes}m)
                                </span>
                            ` : ''}
                        </div>
                    </div>
                    ...
                    <div class="column-main" style="display: flex; flex-direction: column; gap: 1.5rem; width: 100%; box-sizing: border-box;">
                        ...
                        <div class="glass-card" style="padding: 1.5rem; border-radius: var(--radius-md); width: 100%; box-sizing: border-box;">
                            ...
                            <form id="new-reward-form" style="display: flex; flex-direction: column; gap: 0.85rem;">
                                ...
                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.85rem; flex-wrap: wrap;">
                                    <div class="form-group" style="margin-bottom: 0; gap: 4px;">
                                        <label style="font-size: 0.75rem; font-weight: 600; color: var(--text-secondary);">Android App Lock to Bypass (Optional)</label>
                                        <input type="text" id="reward-package-input" class="form-control" placeholder="e.g. com.instagram.android" style="padding: 0.45rem 0.75rem; font-size: 0.82rem;">
                                    </div>
                                    <div class="form-group" style="margin-bottom: 0; gap: 4px;">
                                        <label style="font-size: 0.75rem; font-weight: 600; color: var(--text-secondary);">Bypass Duration (Minutes)</label>
                                        <input type="number" id="reward-duration-input" class="form-control" min="1" max="1440" value="60" style="padding: 0.45rem 0.75rem; font-size: 0.82rem;">
                                    </div>
                                </div>
    ...
    _setupListeners() {
        ...
        if (form) {
            form.addEventListener('submit', (e) => {
                ...
                const packageName = this.container.querySelector('#reward-package-input').value.trim();
                const durationMinutes = parseInt(this.container.querySelector('#reward-duration-input').value);
                ...
                if (name && cost > 0) {
                    ...
                    const newReward = {
                        ...
                        durationMinutes: packageName ? durationMinutes : 60
                    };
                    list.push(newReward);
                    ...
                    this.container.querySelector('#reward-package-input').value = '';
                    this.container.querySelector('#reward-duration-input').value = 60;
                }
            });
        }
        ...
        redeemBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (reward && (blueprints.coins || 0) >= reward.cost) {
                    ...
                    // Call native Capacitor blocker bridge if running on device
                    if (reward.packageName && window.Capacitor && window.Capacitor.Plugins.AppBlocker) {
                        const durationSec = (reward.durationMinutes || 60) * 60;
                        window.Capacitor.Plugins.AppBlocker.unlockApp({
                            packageName: reward.packageName,
                            durationSeconds: durationSec
                        }).then(result => {
                            console.log("App unlocked natively:", result);
                        }).catch(err => {
                            console.error("Native app unlock failed:", err);
                        });
                    }
    ...
    updateView() {
        ...
        this.container.innerHTML = `
            <div class="animate-fade-in" style="width: 100%; box-sizing: border-box; max-width: 600px; margin: 0 auto;">
                ...
                <!-- Android App Blocker Settings -->
                <div class="glass-card" style="padding: 1rem 1.25rem; border-radius: var(--radius-md); margin-bottom: 0.75rem;">
                    <h3 class="card-title" style="font-weight: 500; margin-bottom: 0.75rem; font-size: 1rem; display: flex; align-items: center; gap: 6px;">
                        <i data-lucide="shield-alert" style="color: var(--primary); width: 18px; height: 18px;"></i> Android App Blocker
                    </h3>
                    <div style="display: flex; flex-direction: column; gap: 0.6rem;">
                        <span style="font-size: 0.78rem; color: var(--text-secondary); line-height: 1.4;">
                        ...
                        <div id="blocker-native-status" style="font-size: 0.7rem; color: var(--text-muted); font-weight: 600; display: flex; align-items: center; gap: 4px; margin-top: 2px;">
                            Status: <span id="blocker-status-badge">Checking native bridge...</span>
                        </div>
                    </div>
                </div>
...
function doPost(e) {
  ...
  try {
    ...
    _setupListeners() {
        ...
        // Save App Blocker settings
        const btnSaveBlocker = this.container.querySelector('#btn-save-blocker');
        if (btnSaveBlocker) {
            btnSaveBlocker.addEventListener('click', () => {
                const inputVal = this.container.querySelector('#blocked-packages-input').value.trim();
                db.saveSettings({ blockedPackages: inputVal });
                
                const list = inputVal.split(',').map(s => s.trim()).filter(s => s.length > 0);
                
                if (window.Capacitor && window.Capacitor.Plugins.AppBlocker) {
                    window.Capacitor.Plugins.AppBlocker.setBlockedApps({
                        blockedApps: list
                    }).then(res => {
                        this._showToast("App Blocker rules updated on device!");
                    }).catch(err => {
                        console.error(err);
                        this._showToast("Failed to sync with device: " + err.message);
                    });
                } else {
        ...
        setTimeout(() => {
            const statusBadge = this.container.querySelector('#blocker-status-badge');
            if (statusBadge) {
                if (window.Capacitor && window.Capacitor.Plugins.AppBlocker) {
                    window.Capacitor.Plugins.AppBlocker.getBlockedAppsStatus().then(res => {
                        statusBadge.innerHTML = '<span style="color: #b8f064;">Active (Connected to Android)</span>';
                    }).catch(err => {
                        statusBadge.innerHTML = '<span style="color: #ef4444;">Native Plugin Error</span>';
                    });
                } else {
                    statusBadge.innerHTML = '<span style="color: var(--text-muted);">Not Running in Android Shell (Web Preview)</span>';
                }
            }
        }, 150);
