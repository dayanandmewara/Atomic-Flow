...
// 3. Server Factory Function: Generates isolated Server instances per client session
...
if (isSseMode) {
  ...
  // CORS Middleware for cloud clients
  ...
  // GET Endpoint: Establish SSE Stream per client connection
  app.get("/sse", (req, res) => {
    ...
    // Construct a fully qualified absolute endpoint URL (avoiding double "?" SDK bugs)
    const absoluteEndpoint = `${req.protocol}://${req.get("host")}/messages`;
    
    // Create new transport & fresh isolated server instance for this connection session!
    const clientServer = createServerInstance();
    const transport = new SSEServerTransport(absoluteEndpoint, res);
    const sessionId = transport.sessionId;
    
    activeTransports.set(sessionId, transport);

    clientServer.connect(transport).catch(err => {
  ...
  // POST Endpoint: Handle Tool Call Messages
  app.post("/messages", express.json(), async (req, res) => {
    const connectionId = req.query.connection_id;

    // Validate connection_id is active and registered
    if (MCP_AUTH_KEY) {
  ...
  const localServer = createServerInstance();
