// Netlify Edge Function for AtomicFlow MCP Serverless SSE Broker (Deno / zero-buffering)

const SHEETS_URL = Netlify.env.get("SHEETS_URL");
const MCP_AUTH_KEY = Netlify.env.get("MCP_AUTH_KEY");
...
// 5. Netlify Edge Function Handler
export default async (request, context) => {
  const url = new URL(request.url);
  const method = request.method;

  // Base CORS headers for Deno environment
  const corsHeaders = {
  ...
  // --- GET REQUEST: ESTABLISH SSE TRANSPORT STREAM (Deno zero-buffering native!) ---
  if (method === "GET") {
    ...
    // The Edge Function maps directly to "/mcp" routing
    const clientEndpoint = `${url.origin}/mcp?${clientParams.toString()}`;
